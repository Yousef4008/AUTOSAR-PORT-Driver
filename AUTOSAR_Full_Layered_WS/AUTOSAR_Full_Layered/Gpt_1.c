...
static void (*g_SysTick_Call_Back_Ptr)(void) = NULLPOINTER;
...
void SysTick_Handler(void)
{
    ...
    if(g_SysTick_Call_Back_Ptr != NULLPOINTER)
    {
...
{
    SYSTICK_CTRL_REG    = ZERO;                      /* Disable the SysTick Timer by Clear the ENABLE Bit */
    SYSTICK_RELOAD_REG  = 15999 * Tick_Time;         /* Set the Reload value to count n miliseconds */
    SYSTICK_CURRENT_REG = ZERO;                      /* Clear the Current Register value */
